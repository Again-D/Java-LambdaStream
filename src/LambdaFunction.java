public class LambdaFunction {
    ...
    /*
      [람다식 (간결한 코드를 위해서 사용)]
      고전적 방식인 객체지향적인 프로그래밍 방식이 아닌 함수식 프로그래밍를 위한 기법으로 사용.
      람다는 익명함수를 위한 방식. 람다는 기존방식보다 자바코드를 간결하게 표현할 수 있음.
      이런 장점을 가지고 있습니다. 다만, 처음에는 익숙치 않아서 혼란스러울 수 있음.

      람다식은 함수적 인터페이스를 자료형으로 사용합니다.
      함수적 인터페이스? 추상 메서드가 한개만 존재하는 인터페이스를 가리킴.

      [람다식의 표현]
      (타입매개변수) -> { 실행문들 ...}
      매개변수가 없는 경우에는 () -> {} 식으로 표현 가능.
      매개변수가 한개라면 ()를 생략하여 표현하는 것도 가능함.

      람다식은 함수적 인터페이스를 사용하여
      인터페이스 변수 = 람다식; 같은 구조로 처리를 합니다.

     */

    public static void main(String[] args) {
        // 첫번째 람다식
        InterfaceEx ie = (int a, int b) -> a + b; // {} 생략 이유는 한줄이어서...
...
//        };
        ...
        // 람다식에서 사용되는 this의 의미
        // 람다식은 내부클래스의 익명객체 인식. 내부에 this는 람다 객체 그 자신을 의미함.
        // 즉, 내부클래스와 같이 람다 표현식 내에서 지역변수 접근시 지역변수는 상수화 해야 함.
...
interface LambdaInterface2 {
    void print(String name);  // 매개변수 존재하고, 반환 타입이 없는 경우
}
